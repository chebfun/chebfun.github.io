...
%% Introduction
% The price of a contingent claim can be expressed as the expectation, under
% the so-called risk-neutral measure, of the payoff discounted to present
% value. More precisely, consider a contract of European type, which specifies
% a payoff $V(S_T)$, depending on the level of the underlying asset $S_t$ at
% maturity $t=T$. The value $V$ of such contract at time $t=0$, conditional to
% an underlying value $S_0$ is
%
% \begin{equation}
%   V(S_0) = E^Q[e^{-rT} V(S_T)],  \label{eq1}
% \end{equation}
%
% where $E^Q[\cdot]$ denotes the expectation under the risk-neutral measure
...
% little else than basic probability. (Admittedly, Chebfun makes it look
% particularly simple.)
%
...
% back to Sprenkle in 1961 [5], is the geometric Brownian motion. This is a
...
% lines show the PDF at intermediate times, spaced every 0.05 (the unit of
% time is years).
RHS = 10000;
INT = 'interval';
tvec = 0.05:0.05:1;
...
% Now we make the first step for the evaluation of formula (\ref{eq1}), a
% critical one, which encapsulates the key element of the risk-neutral pricing
% theory: the probability measure of the process governing the asset has to be
% changed in such a way that the resulting process is a martingale. The new
% measure is known as the "risk-neutral measure" and is equivalent (in some
% sense not discussed here) to the original one.
%
...
% profit with no risk), an idea found in embryonic form in the seminal work of
% Black and Scholes [2] and Merton [6], and formalized by Harris and Kreps [3]
% and Harris and Pliska [4].
%
% For all its conceptual complexity, implementing the change of measure in the
% case of a GBM process could not be simpler: it consists of replacing the
% original drift $\mu$ of the stock, with the risk-free rate $r$ of the
% market. We construct again a function handle `lognHnd`, but with $r$ instead
% of $\mu$ and hard-coding the time $t=T$ which is when the call expires.
...
%%
% What is the probability that the option expires OOM? To
% calculate this value, first we use the `cumsum` command to obtain the cumulative
...
% can we calculate the distribution of a random variable which is itself
...
% implies the requirement of the function being monotonically increasing or
% decreasing. The usual way of dealing with this requirement is to split the
% domain of the function in regions with this behaviour and then putting them
% together.
%
...
%%
% Notice that the domain of the ITM density function corresponds to the range
% of $y=S-K$ for $S\geq K$, i.e. $[0,\infty)$. As before, we have used the
% value RHS to specify the right end-point.
%%
...
%%
% The domain of the OOM is the point zero, but we constructed its chebfun
% on the domain `[0,RHS]` so we can add it with the ITM chebfun.
% We can obtain now the PDF of $e^{-rT}V(S_T)$ simply as the discounted
...
hold off
%%
% As before, we can check the accuracy by calculating the area under
% the curve of the undiscounted PDF:
sum(exp(r*T)*payoffPDF)

%% Comparison with the Black-Scholes formula
%
% The final step for the pricing of the call option is the calculation of the
% expected value of the distribution we just obtained:

approx = sum(x.*payoffPDF);
disp(['approx = ', num2str(approx,'%10.15f')])

%%
% We indicate the location of the expected value with a blue
% line on the payoff's distribution.
hold on
plot([approx approx],[0 0.04],'b--',LW,1.6)
hold off
%%
% Notice that since the location of the Dirac delta in this example is at 
% zero, the inclusion of the OOM component makes no difference when 
% computing the expected value and we could have skip its construction.
%%
...
% European call option. The method is conceived in the framework of risk-
% neutral pricing theory, and by "computing with density functions instead of
% numbers", we have managed to avoid the Monte Carlo simulation approach.
%
% The easy implementation in Chebfun and the high accuracy obtained are
% promising features but further experiments need to be done, with different
% contracts and distributions, to have a better understanding of its pros and
% cons.
%
...
%    to obtain $dx/dy$; obtain the composition $f(x(y))$ and calculate the
%    contributions with formula (\ref{eq2}).
%
