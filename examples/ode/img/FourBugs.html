...
layout: example
authordate: "Hrothgar, 5th November 2013"
meta: "(Chebfun example ode/FourBugs.m) [Tags: #nonlinearODE]"
---
...
title('The "Four bugs on a rectangle" problem', FS, 14)</pre><img src="img/FourBugs_01.png" alt="">
...
u = chebfun.ode113(fun, interval, initial_conds, opts);</pre>The variable `u` is now a 4-column quasimatrix containing the solution to the system of differential equations. With some simple transformations we can reconstruct the paths of the four bugs in the complex plane.
...
bugs(:,3) = -bugs(:,1);  bugs(:,4) = -bugs(:,2);  bugs(:,5) = bugs(:,1);</pre>Here is a plot of their movement toward the origin.

<pre class="mcode-input">figure(2)
plot(bugs, LW, 2), hold on
plot(xx, yy, 'k.', MS, 18), axis equal off
title('The dance of four bugs on a rectangle', FS, 14)</pre><img src="img/FourBugs_02.png" alt="">
...
<pre class="mcode-input">figure(2), hold on
for t = linspace(-1.5, -.01, 7),
    plot(bugs(t,:), 'k.-', MS, 18)
end
ylim([-.55 .55])
title('The dance of four bugs on a rectangle, with level sets', FS, 14)</pre><img src="img/FourBugs_03.png" alt="">

We can verify in Chebfun that the perimeter of the bugs' parallelogram decreases at the constant rate 4.

<pre class="mcode-input">figure(3)
subplot(2,1,1)
...
subplot(2,1,2)
    ...
    ylim([-7 0])
    title('Rate of change of parallelogram perimeter', FS, 14)</pre><img src="img/FourBugs_04.png" alt="">
...
fprintf('{t_rect} estimated by Chebfun:      %-2.16f    %-16.16f\n', t_rect_est)</pre><pre class="mcode-output">{t_rect} exact (Chapman et al.):    -1.5000000000000000    -0.148243863926 
{t_rect} estimated by Chebfun:      -1.5000000000000000    -0.1482438639260601
</pre>Our second computed answer agrees to every digit with the time given in [1], and there is a chance that Chebfun has does even better. (The tolerance of the ODE solver was set to 5e-14, so in fact we should expect that Chebfun has done one digit better.)
...
fprintf('{t_rhomb} estimated by Chebfun:      %-16.16f    %-16.16f\n', t_rhomb_est)</pre><pre class="mcode-output">{t_rhomb} exact (Chapman et al.):    -0.328190029216        -0.011365703053
{t_rhomb} estimated by Chebfun:      -0.3281900292157962    -0.0113657030533104
</pre>Again, Chebfun gives an answer correct to as many digits as are available.
...
<pre class="mcode-input">figure(4)
...
subplot(2,2,4), hold on
    ...
    title(['rhombus, t = ' num2str(tt(4),10)], FS, 12)</pre><img src="img/FourBugs_05.png" alt="">
