---
title: "Mercury-Earth Conjunctions"
layout: example
authordate: "Tonatiuh Sanchez-Vizuet and Matthew Moye, 17th June 2012"
meta: "(Chebfun example linalg/MercuryEarthConjunctions.m) [Tags: #linearalgebra, #rootfinding, #determinant]"
---
...
<pre class="mcode-input">M = @(t) [-11.9084 + 57.9117 * cos(2*pi*t/87.97),...
           56.6741 * sin(2*pi*t/87.97);
          -2.4987 + 149.6041 * cos(2*pi*t/365.25),...
           149.5832 * sin(2*pi*t/365.25)];

f = chebfun(@(t) det(M(t)),[0 600],'vectorize');</pre>The roots of the determinant give the days at which a conjunction occurs.

<pre class="mcode-input">zeros = roots(f);</pre>To get a visual interpretation of the roots, one can plot the determinant and then plot _zeros_ all at value $0$. The following figure depicts the times of the first ten conjunctions.

<pre class="mcode-input">figure
plot(f,'linewidth',1.6), hold on, grid on
plot(zeros(1:10),0,'.r','markersize',20)
xlabel('Time(days)')</pre><img src="img/MercuryEarthConjunctions_01.png" alt="">
