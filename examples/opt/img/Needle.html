...
close all, plot(h,LW,1), axis equal, axis([-4 4 -.4 2])</pre><img src="img/Needle_01.png" alt="">
...
For us, when I hear the word "continuum", we think Chebfun. Suppose we specify two variables: $x$, the horizontal position of the center of the needle, and $\theta$, its angle counterclockwise from the horizontal.  Given $x$ and $\theta$, we then ask how low the needle can lie.  Let
...
end</pre>For example, here are needle positions for $(x,\theta) = (-.6, -0.2)$ and $(x,\theta) = (1.7, 1)$.

<pre class="mcode-input">subplot(2,1,1)
plotneedle(-0.6,-0.2), title('needle with (x,theta) = (-.6, -0.2)',FS,14)
subplot(2,1,2)
plotneedle(1.7,1), title('needle with (x,theta) = (1.7, 1)',FS,14)</pre><img src="img/Needle_02.png" alt="">
...
plot(xvec(1),yval,'.k',MS,12), grid on</pre><img src="img/Needle_05.png" alt="">
