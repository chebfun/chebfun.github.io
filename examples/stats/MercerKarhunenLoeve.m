...
%%
% Mercer's theorem is a continuous analog of the singular-value or eigenvalue
% decomposition of a symmetric positive definite matrix. One of its main
% applications is to find convenient ways to express stochastic processes, via
% the Karhunen-Loeve expansion [1].

%% Mercer's theorem
% Suppose $K(s,t)$ is a symmetric (that is, $K(t,s)=K(s,t)$), continuous, and
...
%%
% The truncation of the Mercer sum does lead to an underestimate of the values
% of the kernel $K(s,t)$. For our example, we should get $K(s,s)=1$, but we
% get noticeably less.
...
% interval $[a,b]$ and $\omega$ in some probability space. The process is
% often characterized by its mean, $\mu(t)$, and its covariance, $K(s,t)$, the
% expected value of $(X(s)-\mu(s))(X(t)-\mu(t))$. Using Mercer's theorem on
% $K$, we can express the process by the K-L expansion
%
% $$ X(t,\omega) = \mu(t) + \sum_j^\infty \sqrt(\lambda_j) \psi_j(t)
% Z_j(\omega), $$
%
% where $\lambda_j$ and $\psi_j$ are Mercer eigenmodes for $K$, and the $Z_j$
% are uncorrelated and of unit variance.

%%
% K-L is a generalization of the singular value decomposition of a matrix,
% which can be written as a sum of outer products of vectors. The covariance
% $K$ plays the role of the Gram matrix inner products (in probability) of
% "columns" of the process for different values of $s$ and $t$. A number of
% SVD results have K-L analogs, most notably that the best approximation of
% the process results from truncating the expansion, if the eigenvalues are
...
% calculate the variance in a truncation of the expansion by summing only some
% of the eigenvalues. For example, suppose the process $X$ has the exponential
% covariance in $K$ above. The eigenvalues show that $95\%$ of the variance in
% the process is captured by the first $10$ K-L modes:
...
%%
% If we shorten the correlation length of the process relative to the domain
