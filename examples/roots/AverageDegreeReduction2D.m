...
%% 2D subdivision
% Generally, Chebfun2 approximates globally smooth functions $f(x,y)$ by
% global low rank polynomial interpolants. However, in the bivariate
% rootfinding algorithm to solve
%
% $$ f(x,y) = g(x,y) = 0, $$
%
% we use piecewise smooth interpolants if the polynomial degrees are larger
% than $16$ in the $x$- or $y$-variable (of $f$ or $g$). In bivariate
% rootfinder the resultant based method recursively subdivides the rectangular
% domain and the functions $f(x,y)$ and $g(x,y)$ are approximated by piecewise
% smooth bivariate polynomial interpolants with each piece of degree at most
% $16$. Subdivision is useful for reducing the complexity of the bivariate
% rootfinding algorithm. The reduction in the complexity is determined by a
% parameter $\tau$ that measures the average degree reduction. Essentially
% subdivision means that much higher degree problems can be solved (see
% [1,2]).

%% The average degree reduction parameter
% The parameter $\tau$ measures the average reduction in polynomial degrees of
% $f(x,y)$. That is, if a polynomial of degree $n$ (in the $x$- and
% $y$-variable) is required to approximate $f(x,y)$ on $[-1,1]\times[-1,1]$
% then the average degree required to approximate $f$ on $[-1,r]\times[-1,s]$
% (and the other subdomains) is $\tau n$, where $r$ and $s$ are two small
% arbitrary constants. Throughout this Example we take symmetric functions,
% i.e., $f(x,y) = f(y,x)$ since then the degree reduction is identical in the
% $x$ and $y$ direction, which considerably simplifies the discussion. The
% average degree reduction parameter was introduced in [1].

%% Rank one functions
% A function $f(x,y)$ is of rank $1$ if it can can be written as a product of
% univariate functions, i.e., $f(x,y) = h(x)k(y)$. Since in this example we
% are considering only symmetric functions we have $f(x,y) = h(x)h(y)$. For
% rank $1$ functions the average degree reduction parameter is typically about
% $1/2$ because under subdivision the degree reduction is directly determined
% by the degree reduction in $h(x)$. The average degree reduction parameter
% for univariate functions is discussed in [2]. For example,
...
% oscillations of $f(x,y)$ in a rectangular domain $[a,b]\times[c,d]$ is
% directly proportional to $(b-a)(d-c)$. Moreover, the number of oscillations
...
% parameter $\tau$ then the rank $1$ function $f(x,y) = h(x)h(y)$ has the same
% average degree reduction parameter.

%% Toeplitz functions
% A function $f(x,y)$ on $[-1,1]\times[-1,1]$ is defined as Toeplitz if there
% is a univariate function $h(t)$ on $[-2,2]$ such that $f(x,y) = h(x-y)$.
% Functions in this class are typically of rank at least two and the average
% degree reduction parameter cannot be understood from the univariate setting.
...
% $[a,b]\times[a,b]$, where $0<a<b$. This is a 2D generalisation of functions
% of the form $1/(x+c)$, which has been used to investigate the parameter
% $\tau$ in the univariate case [2]. The numerical degree of $f(x,y)$ can be
% determined by using Eliott's method for all $0<a<b$.
