...
<pre class="mcode-input">  F = @(x) abs(besselj(0,x));
  ...
  plot(f), ylim([0 1.1])</pre><img src="img/guide2_01.png" alt="">
...
<pre class="mcode-input">  x = chebfun('x');
  ...
  g = sin(9*x);
  h = min(f,g);
  plot(h)</pre><img src="img/guide2_02.png" alt="">
...
<pre class="mcode-input">  plot(f)</pre><img src="img/guide2_03.png" alt="">
...
<pre class="mcode-input">  f = chebfun('exp(-1./sin(10*x).^2)');
  plot(f)</pre><img src="img/guide2_04.png" alt="">
...
<pre class="mcode-input">  t = chebfun('t',[-5 5]);
  ...
  fint = cumsum(f);
  plot(fint,'m')
  ylim([-0.2 2.2]), grid on</pre><img src="img/guide2_05.png" alt="">
...
<pre class="mcode-input">  fint = fint - fint(0);
  plot(fint,'m')
  ylim([-1.2 1.2]), grid on</pre><img src="img/guide2_06.png" alt="">
...
<pre class="mcode-input">  x = chebfun('x',[0 6]);
  ...
  g = cumsum(f); subplot(1,2,2), plot(g,'m')</pre><img src="img/guide2_07.png" alt="">
...
<pre class="mcode-input">  clf, plot(Li,'m')
  p = primes(xmax);
  hold on, plot(p,1:length(p),'.k')</pre><img src="img/guide2_08.png" alt="">
...
<pre class="mcode-input">  f = chebfun('cos(pi*x)',[0 20]);
  fprime = diff(f);
  hold off, plot(f)
  hold on, plot(fprime,'r')</pre><img src="img/guide2_09.png" alt="">
...
<pre class="mcode-input">  f = chebfun({@(x) x.^2, @(x) 1+0*x, @(x) 4-x, @(x) 4./x},0:4);
  hold off, plot(f)</pre><img src="img/guide2_10.png" alt="">
...
<pre class="mcode-input">  f = chebfun('1./(1+x.^2)');
  g = diff(f,4); plot(g)</pre><img src="img/guide2_11.png" alt="">
...
<pre class="mcode-input">  r = @(x,y) sqrt(x.^2+y.^2); theta = @(x,y) atan2(y,x);
  ...
  axis([-2 2 0.5 2.5]), colorbar, grid on</pre><img src="img/guide2_12.png" alt="">
