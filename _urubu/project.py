...
from __init__ import UrubuWarning, UrubuError
import readers, processors
...
class Project(object):
    ...
    def make_fileinfo(self, relfn, meta):
        ...
        info['id'] = self.make_id(components)
        # make html url from ref
        info['url'] = self.site['baseURL'] + info['id'] + self.site['link_ext']
        info.update(meta)
        return info
    ...
    def make_navinfo(self, relpath, meta):
        ...
        info['url'] = self.site['baseURL'] + info['id']
        if info['url'] != '/':
            info['url'] += '/'
        info.update(meta)
        return info
    ...
    def resolve_reflinks(self):
        for info in self.navinfo:
            ...
            index_id = self.make_id(info['components'] + ['index'])
